...
function AuthAwareDemo() {
  ...
  const plugins = [
    createAuthAwarePlugin({
      ...
      onUnauthorized: () => {
        setEvents((prev) => [`[${new Date().toLocaleTimeString()}] onUnauthorized fired`, ...prev.slice(0, 4)]);
      },
  ...
  return (
    <div>
      <div style={{ width: "clamp(32rem, 80vw, 72rem)", aspectRatio: "16/9", position: "relative", background: "#000", borderRadius: "0.8rem", overflow: "hidden" }} data-f8-player="" data-theme="minimal">
      ...
      <div style={{ marginTop: "1.2rem", background: "#111", color: "#aaa", fontFamily: "monospace", fontSize: "1.2rem", padding: "1.2rem", borderRadius: "0.6rem", minHeight: "6rem" }}>
        <div style={{ color: "#555", marginBottom: "0.6rem" }}>Auth events (triggered by 401/403 on allowlisted domains):</div>
        {events.length === 0 ? <span style={{ color: "#555" }}>— no events yet —</span> : events.map((e, i) => <div key={i}>{e}</div>)}
      </div>
    </div>
  );
}
