...
- # DX flexibility improvements (Phase 2–9)
  ...
  ## `@f8team/reel-react` — callback props + plugin commands + i18n
...
onEnded / onError / onReady / onProgress / onQualityChange / onUnauthorized`
  ...
  - **i18n** — `defaultLabels`, `vietnameseLabels`, `useLabels()`,
    ...
    `<Player.Root>` (or `<ReelWebPlayer>`) to override every ARIA string and the
    time-formatter; `vietnameseLabels` reproduces the previous hardcoded
    Vietnamese strings byte for byte.

  ## `@f8team/reel-plugin-prefs` — new public API
  ...
  ## `@f8team/reel-preset-web` — factory bundles for Reel web apps

  New package exposing three consumption patterns:
  - **`createReelWebPlayerPlugins(opts?)`** — returns a ready-to-use plugin list
    (`ReelWebPlayerPluginsOptions`). Use with your own `createPlayer` call.
  - **`<ReelWebPlayer>`** (React) — one-liner React component with all Reel web
    plugins pre-wired (`ReelWebPlayerProps`, `ReelWebPlayerLight`).
  - **`<reel-web-player>` / `defineReelWebPlayer()`** (Lit) — one-liner custom
    element for Lit / vanilla HTML (`ReelWebPlayerElement`).
  - Re-exports `vietnameseLabels`, `defaultLabels`, `PlayerLabels`,
    `PlayerCallbackProps` from `@f8team/reel-react` so consumers have a single
    import point.

  ## `@f8team/reel-themes` — headless CSS layer
...
- Updated dependencies
  ...
  - @f8team/reel-react@1.0.0
  - @f8team/reel-lit@1.0.0
  - @f8team/reel-plugin-keyboard@1.0.0
  - @f8team/reel-plugin-auth-aware@1.0.0
  - @f8team/reel-plugin-prefs@1.0.0
  - @f8team/reel-plugin-fullscreen@1.0.0
  - @f8team/reel-plugin-hls-quality@1.0.0
  - @f8team/reel-plugin-markers@1.0.0
  - @f8team/reel-plugin-pip@1.0.0
  - @f8team/reel-plugin-subtitles@1.0.0
  - @f8team/reel-plugin-thumbnails@1.0.0
