import type { PluginInstance } from "@f8team/reel-core";
import type { AuthAwarePluginOptions } from "@f8team/reel-plugin-auth-aware";
import { createAuthAwarePlugin } from "@f8team/reel-plugin-auth-aware";
import { createFullscreenPlugin } from "@f8team/reel-plugin-fullscreen";
import { createHlsQualityPlugin } from "@f8team/reel-plugin-hls-quality";
import type { KeyboardPluginOptions } from "@f8team/reel-plugin-keyboard";
import { createKeyboardPlugin } from "@f8team/reel-plugin-keyboard";
import type { MarkersPluginOptions } from "@f8team/reel-plugin-markers";
import { createMarkersPlugin } from "@f8team/reel-plugin-markers";
import { createPipPlugin } from "@f8team/reel-plugin-pip";
import type { PrefsPluginOptions } from "@f8team/reel-plugin-prefs";
import { createPrefsPlugin } from "@f8team/reel-plugin-prefs";
import type { SubtitlesPluginOptions } from "@f8team/reel-plugin-subtitles";
import { createSubtitlesPlugin } from "@f8team/reel-plugin-subtitles";
import type { ThumbnailsPluginOptions } from "@f8team/reel-plugin-thumbnails";

/** Canonical keyboard defaults — matches f8-pro-ui `<video-player>` inner mount. */
export const DEFAULT_REEL_KEYBOARD: KeyboardPluginOptions = {
  scope: "global",
  seekStep: 5,
  longSeekStep: 10,
};
 ...
 */
export const DEFAULT_REEL_GATEWAY_ALLOWLIST: NonNullable<AuthAwarePluginOptions["allowlist"]> = [
  /^https:\/\/api-gateway\./,
];

export interface ReelWebPlayerPluginsOptions {
  /**
   ...
   * default storage key `"reel-player:prefs"`).
   *
...
/**
 * Build the standard Reel-web plugin tuple in a stable merge order across apps.
 *
 * Order: markers → keyboard → [hls-quality] → auth → fullscreen → pip → subtitles → thumbnails.
 */
export function createReelWebPlayerPlugins(options: ReelWebPlayerPluginsOptions): PluginInstance[] {
  const {
    markers,
    keyboard,
    hlsQuality = true,
  ...
  if (keyboard !== false) {
    const kOpts = keyboard === undefined ? DEFAULT_REEL_KEYBOARD : keyboard;
  ...
  return {
    ...
    setup(player, host) {
      ...
      const load = (): void => {
        ...
        loading = true;
        import("@f8team/reel-plugin-thumbnails")
