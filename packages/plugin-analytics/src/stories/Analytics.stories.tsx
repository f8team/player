...
function AnalyticsDemo() {
  ...
  const plugins = [
    createAnalyticsPlugin({
      sink(event: AnalyticsEvent) {
        const line = `[${new Date().toLocaleTimeString()}] ${event.type}${
          "currentTime" in event ? ` @ ${(event as { currentTime: number }).currentTime.toFixed(1)}s` : ""
        }`;
  ...
  return (
    <div style={{ display: "flex", gap: "2rem", alignItems: "flex-start", flexWrap: "wrap" }}>
      <div style={{ width: "clamp(28rem, 60vw, 56rem)", aspectRatio: "16/9", position: "relative", background: "#000", borderRadius: "0.8rem", overflow: "hidden" }} data-f8-player="" data-theme="classroom">
      ...
      <div style={{ minWidth: "22rem", background: "#111", color: "#0f0", fontFamily: "monospace", fontSize: "1.2rem", padding: "1.2rem", borderRadius: "0.6rem", maxHeight: "18rem", overflow: "auto" }}>
        <div style={{ color: "#888", marginBottom: "0.8rem" }}>Analytics events:</div>
        {events.length === 0 && <div style={{ color: "#555" }}>— play the video to see events —</div>}
