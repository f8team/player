...
function makePlayer(overrides: {
  ...
  const { currentTime = 30, duration = 100, src = "https://cdn.example.com/video.m3u8" } = overrides;
  const handlers: Partial<{ [K in keyof PlayerEvents]: Handler<K>[] }> = {};
  const state = { currentTime, duration };

  const player = {
    getState: () => ({
      status: "idle" as const, source: { src }, currentTime: state.currentTime,
      duration: state.duration, buffered: [], playbackRate: 1, volume: 1, muted: false,
    ...
    setVolume: vi.fn(), setMuted: vi.fn(), setSource: vi.fn(), getCurrentTime: vi.fn(),
...
describe("createResumePositionPlugin", () => {
  ...
  it("does not seek when saved position is below minSeconds", () => {
    ...
    createResumePositionPlugin({ storage, minSeconds: 3 }).setup(
      player as unknown as Player, makeHost(),
    );
    player.fire("ready", { duration: 100 });
